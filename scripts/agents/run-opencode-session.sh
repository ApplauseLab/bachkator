...
fresh_session="${OPENCODE_FRESH_SESSION:-}"

mkdir -p "$session_dir" "$(dirname "$log_file")" "$(dirname "$checkpoint_file")"

if [ -n "$fresh_session" ] && [ "$fresh_session" != "0" ]; then
  fresh_token=$(date -u '+%Y%m%dT%H%M%SZ')
  session_title="$session_title/fresh-$fresh_token"
  session_file="$session_dir/$role-$slug-$prompt_hash-fresh-$fresh_token.env"
fi

if { [ -z "$fresh_session" ] || [ "$fresh_session" = "0" ]; } && [ -f "$session_file" ]; then
...
if { [ -z "$fresh_session" ] || [ "$fresh_session" = "0" ]; } && [ -z "$session_id" ]; then
...
started_at=$(date -u '+%Y-%m-%dT%H:%M:%SZ')
{
  ...
  if [ -n "$fresh_session" ] && [ "$fresh_session" != "0" ]; then
    printf 'checkpoint.fresh_session=true\n'
  fi
