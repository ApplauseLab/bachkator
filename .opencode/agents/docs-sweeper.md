...
6. When participating as a Bach policy reviewer, emit findings when documentation is not updated for user-visible or agent-visible changes. Missing or stale docs for Bachfile syntax, CLI behavior, quality/policy contracts, agent prompts, provider execution, managed-control-plane exports, or examples should be reported as policy-relevant findings rather than only prose suggestions.
...
5. Fix when appropriate.
   ...
   - Keep edits minimal, accurate, and consistent with existing style.
    ...
    - If a docs update is required but outside your allowed scope, emit a finding with the missing doc surface and evidence.
6. Report clearly.
