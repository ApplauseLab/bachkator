...
Trigger Provider, Backend, and Approval Provider protocols are part of the factory roadmap. The Approval Provider protocol (`bach.approval.v1`) ships alongside Factory approval gates. Notification Providers, Command Providers, and Deployment Providers remain planned Integration Provider families whose protocols should not be designed until their local semantics are proven.
...
Approval Providers submit evidence through the same Bach approval path and validation rules as CLI approvals; see the Factory Approval Providers reference for the `bach.approval.v1` protocol contract. External approval evidence is not a separate state transition mechanism.
...
- Document provider trigger Bachfile syntax and protocol schema in reference docs.
