...
func (c *Client) Handshake(ctx context.Context, params HandshakeParams) (HandshakeResult, error) {
	...
	err := c.call(ctx, "approval.handshake", params, &result)
	return result, err
}

func (c *Client) Poll(ctx context.Context, params PollParams) (PollResult, error) {
	var result PollResult
	err := c.call(ctx, "approval.poll", params, &result)
	return result, err
}
...
func (c *Client) call(ctx context.Context, method string, params any, result any) error {
	...
	if err != nil {
		if errors.Is(err, io.EOF) {
			return fmt.Errorf("approval provider closed stdout during %s", method)
		}
		...
		if response.Error.Data != nil {
			...
			if err == nil {
				...
				if err := json.Unmarshal(encoded, &domainErr); err == nil && domainErr.Code != "" {
					return fmt.Errorf("approval %s: %w", method, domainErr)
				}
			}
		}
		return fmt.Errorf("approval %s: %s", method, response.Error.Message)
	}
