...
const (
	...
	TargetTypeAgent    = model.TargetTypeAgent
...
type AgentSpec = model.AgentSpec
type ImprovePolicy = model.ImprovePolicy
...
func (t *Target) Spec() TargetSpec {
	return TargetSpec{
		...
		Quality: TargetQuality{
			...
			RegoPolicies: regoPolicySpecs(t.RegoPolicies),
...
func (t *Target) body() TargetBody {
	...
	case TargetTypeAgent:
		return AgentSpec{
			Mode:        t.Mode,
			Template:    t.Template,
			Provider:    providerSpec(t.ProviderConfig),
			Role:        t.Role,
			Prompt:      promptSpec(t.PromptConfig),
			Plan:        t.Plan,
			Subject:     t.Subject,
			SubjectInfo: t.AgentSubject,
			Policy:      t.AgentPolicy,
			Workspace:   agentWorkspaceSpec(t.Workspace),
			Git:         agentGitSpec(t.Git),
			Improve:     improvePolicySpec(t.Improve),
		}
...
func agentWorkspaceSpec(workspace []*AgentWorkspaceBlock) model.AgentWorkspace {
	if len(workspace) == 0 || workspace[0] == nil {
		return model.AgentWorkspace{}
	}
	return model.AgentWorkspace{Mode: workspace[0].Mode, Path: workspace[0].Path}
}

func agentGitSpec(git []*AgentGitBlock) model.AgentGit {
	if len(git) == 0 || git[0] == nil {
		return model.AgentGit{}
	}
	return model.AgentGit{Branch: git[0].Branch, Commit: git[0].Commit}
}

func providerSpec(provider *Provider) model.Provider {
	if provider == nil {
...
func promptSpec(prompt *Prompt) model.Prompt {
	if prompt == nil {
		return model.Prompt{}
	}
	return model.Prompt{
...
func improvePolicySpec(blocks []*ImproveBlock) ImprovePolicy {
	if len(blocks) == 0 || blocks[0] == nil {
		return ImprovePolicy{}
	}
	return ImprovePolicy{MaxAttempts: blocks[0].MaxAttempts, Until: blocks[0].Until}
}
...
func regoPolicySpecs(policies []*RegoPolicyBlock) []model.RegoPolicySpec {
	if len(policies) == 0 {
		return nil
	}
	specs := make([]model.RegoPolicySpec, 0, len(policies))
	for _, policy := range policies {
		if policy == nil {
			continue
		}
		specs = append(specs, model.RegoPolicySpec{
			Path:     policy.Path,
