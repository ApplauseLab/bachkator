...
`)
	...
	if len(plan.RequestedTargets) != 1 || plan.RequestedTargets[0] != "shell/deploy" {
		t.Fatalf("requested targets = %#v, want shell/deploy", plan.RequestedTargets)
	}
...
func TestDryRunJSONPrintsMultipleRequestedTargets(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, filepath.Join(dir, "Bachfile"), `project "example" {}
...
`)
	...
	if plan.Target != "shell/lint shell/test" {
		t.Fatalf("target = %q, want shell/lint shell/test", plan.Target)
	}
	if len(plan.RequestedTargets) != 2 || plan.RequestedTargets[0] != "shell/lint" ||
		plan.RequestedTargets[1] != "shell/test" {
		t.Fatalf("requested targets = %#v", plan.RequestedTargets)
	}
	wantOrder := []string{"shell/setup", "shell/lint", "shell/test"}
	if len(plan.TargetOrder) != len(wantOrder) {
		t.Fatalf("target order = %#v, want %#v", plan.TargetOrder, wantOrder)
	}
	for i, want := range wantOrder {
		if plan.TargetOrder[i] != want {
			t.Fatalf("target order = %#v, want %#v", plan.TargetOrder, wantOrder)
		}
	}
}
...
`)
	...
	args := []string{"-f", filepath.Join(dir, "Bachfile"), "run", "--json", "shell/test"}
	if err := Execute(
		context.Background(),
		args,
		&bytes.Buffer{},
