...
func BuildPlan(project *Project, name string) (*Plan, error) {
	return BuildPlanForTargets(project, []string{name})
}

func BuildPlanForTargets(project *Project, names []string) (*Plan, error) {
	return BuildPlanForTargetsWithHandlers(project, names, targetpkg.BuiltinTargetRegistry())
}

func BuildPlanForTargetsWithHandlers(
	project *Project,
	names []string,
	...
	for _, name := range names {
		...
		if canonicalName == "" || seen[canonicalName] {
			continue
		}
	...
	plan := &Plan{
		TargetName:              strings.Join(canonicalNames, " "),
		RequestedTargets:        append([]string(nil), canonicalNames...),
	...
	compositeEdges, err := plan.compositeEdges(targets)
	if err != nil {
		return nil, err
	}
	plan.CompositeEdges = compositeEdges
	plan.PipelineEdges = plan.compositeEdgesOfKind("pipeline_step")
	plan.GroupEdges = plan.compositeEdgesOfKind("group_member")
...
func (p *Plan) ScheduledTargets() map[string]*Target {
	...
	for _, targetName := range p.RequestedTargets {
		p.collectScheduledTarget(targetName, names)
	}
...
func (p *runPlanner) visit(name string) error {
	...
	children, err := targetCompositeChildren(p.targetHandlers, target)
	if err != nil {
		return err
	}
	for _, child := range children {
		if err := p.visit(child.Target); err != nil {
			return err
		}
	}
...
func (p *Plan) compositeEdges(targets TargetHandlers) ([]PlanTypedEdge, error) {
	orderIndex := p.orderIndex()
	edges := []PlanTypedEdge{}
	for _, name := range p.Order {
		children, err := targetCompositeChildren(targets, p.Targets[name])
		if err != nil {
			return nil, err
		}
		for _, child := range children {
			if _, ok := p.Targets[child.Target]; ok {
				edges = append(edges, PlanTypedEdge{From: child.Target, To: name, Kind: child.Kind})
			}
		}
	}
	sort.SliceStable(edges, func(i, j int) bool {
...
func (p *Plan) compositeEdgesOfKind(kind string) []PlanEdge {
	edges := []PlanEdge{}
	for _, edge := range p.CompositeEdges {
		if edge.Kind == kind {
			edges = append(edges, PlanEdge{From: edge.From, To: edge.To})
		}
	}
	return edges
}

func (p *Plan) compositeChildrenOfKind(name string, kind string) []string {
	children := []string{}
	for _, edge := range p.CompositeEdges {
		if edge.To == name && edge.Kind == kind {
...
func (p *Plan) hasCompositeScope(name string) bool {
	for _, edge := range p.CompositeEdges {
		if edge.To == name && (edge.Kind == "pipeline_step" || edge.Kind == "group_member") {
			return true
		}
	}
	return false
