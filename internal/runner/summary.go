...
func formatRunSummary(project *Project, run RunRecord) string {
	...
	if run.Status == model.RunStatusFailed ||
		run.Status == model.RunStatusPreflightFailed ||
		run.Status == model.RunStatusQualityFailed {
...
func firstFailedTarget(run RunRecord) (string, string) {
	...
	for name, target := range run.Targets {
		if target.Status == model.RunStatusFailed ||
			target.Status == model.RunStatusPreflightFailed ||
			target.Status == model.RunStatusQualityFailed {
			failed = append(
				failed,
