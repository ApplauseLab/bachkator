...
type Session struct {
	// Immutable context set at creation; no lock required.
	runner     *Runner
	targets    TargetHandlers
	project    *Project
	backend    *backendpkg.Client
	state      *State
	run        *RunRecord
	plan       *Plan
	...
	// stateMu protects mutable run/session state: fingerprints, dirtyTargets,
	// run/target records, execGraph node state, scope maps, and lock reservations.
	stateMu      sync.Mutex
	...
	// outputMu protects serialized console/log output writers.
	outputMu sync.Mutex
...
func (s *Session) completeRun(ctx context.Context, status model.RunStatus) error {
	s.run.Status = status
	finishedAt := s.now()
	s.run.FinishedAt = finishedAt
	s.run.Artifacts = indexRunArtifacts(s.project, *s.run, s.plan.Targets, finishedAt)
	if !s.runner.DryRun {
		if err := s.backend.Runs.Finish(ctx, s.dirtyTargets, *s.run); err != nil {
			return err
		}
	}
	s.printRunSummary()
...
func (s *Session) completeFailedRun(ctx context.Context, err error) error {
	status := model.RunStatusFailed
	if quality.IsGateError(err) || quality.IsParseError(err) {
		status = model.RunStatusQualityFailed
	}
	if completeErr := s.completeRun(ctx, status); completeErr != nil {
		return completeErr
	}
	return err
}

func (s *Session) completeCheckFailedRun(ctx context.Context, status model.RunStatus) {
	s.run.Status = status
	finishedAt := s.now()
	s.run.FinishedAt = finishedAt
	s.run.Artifacts = indexRunArtifacts(s.project, *s.run, s.plan.Targets, finishedAt)
	if !s.runner.DryRun {
		_ = s.backend.Runs.Finish(ctx, nil, *s.run)
	}
	s.printRunSummary()
}
...
func (s *Session) finishTarget(target string, status model.RunStatus) {
	s.finishTargetWithExitCode(target, status, nil)
}

func (s *Session) finishTargetWithExitCode(target string, status model.RunStatus, exitCode *int) {
	s.stateMu.Lock()
...
func (s *Session) recordPreflightFailure(failure PreflightFailure, message string) {
	for _, targetName := range failure.Targets {
		s.recordSyntheticTargetFailure(
			targetName,
			model.RunStatusPreflightFailed,
...
func (s *Session) closeActiveScopes(status model.RunStatus) {
	s.stateMu.Lock()
