...
func buildExecutionGraph(plan *Plan) (*executionGraph, error) {
	...
	for _, targetName := range plan.RequestedTargets {
		if _, err := b.addClosure(targetName); err != nil {
			return nil, err
		}
	}
...
func (b *executionGraphBuilder) addClosure(name string) (map[string]bool, error) {
	...
	if steps := b.plan.compositeChildrenOfKind(name, "pipeline_step"); len(steps) > 0 {
	...
	if members := b.plan.compositeChildrenOfKind(name, "group_member"); len(members) > 0 {
		b.stack = append(b.stack, name)
		if err := b.addGroup(
			name,
			target.DependsOn,
...
func (b *executionGraphBuilder) addTarget(name string, target *Target) {
	...
	if b.plan.hasCompositeScope(name) {
		b.scopes[name] = true
	}
