...
func TestParseAgentReportV1AcceptsHelperFindingShape(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "agent-report-v1.json")
	data := []byte(`{
  "schema": "bach.agent_report.v1",
  "agent": { "role": "docs-sweeper" },
  "status": "success",
  "summary": "reviewed docs",
  "metrics": [],
  "findings": [{
    "kind": "docs",
    "severity": "error",
	...
	report, err := parseAgentReportV1(path)
	if err != nil {
		t.Fatal(err)
	}
	if len(report.Findings) != 1 || report.Findings[0].DurationMS != 12.5 {
		t.Fatalf("findings = %#v", report.Findings)
	}
}

func TestParseAgentReportV1RejectsTrailingJSON(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "agent-report-v1.json")
	data := []byte(`{"schema":"bach.agent_report.v1","agent":{"role":"docs"}} {}`)
	if err := os.WriteFile(path, data, 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := parseAgentReportV1(path); err == nil {
		t.Fatalf("parseAgentReportV1() error = nil, want trailing JSON error")
	}
}

func TestDecodePluginQualityRejectsTrailingJSON(t *testing.T) {
	data := []byte(`{"metrics":[{"name":"issues.total.count","value":0}]} {}`)
	if _, err := decodePluginQuality(data); err == nil {
		t.Fatalf("decodePluginQuality() error = nil, want trailing JSON error")
	}
}
