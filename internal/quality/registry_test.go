...
func TestReportParserRegistryZeroValue(t *testing.T) {
	var registry ReportParserRegistry
	if _, err := registry.Parser("missing"); err == nil {
		t.Fatal("zero-value missing parser returned no error")
	}
	if err := registry.Register("junit-xml", parserFunc(parseJUnitXML)); err != nil {
		t.Fatal(err)
	}
	if _, err := registry.Parser("junit-xml"); err != nil {
		t.Fatal(err)
	}
}
...
func TestGateRegistryZeroValue(t *testing.T) {
	var registry GateRegistry
	if _, err := registry.Evaluator("missing"); err == nil {
		t.Fatal("zero-value missing gate evaluator returned no error")
	}
	if err := registry.Register("threshold", evaluateThresholdGates); err != nil {
		t.Fatal(err)
	}
	if _, err := registry.Evaluator("threshold"); err != nil {
		t.Fatal(err)
	}
}
