...
func ParseReport(req ParseRequest) (state.QualityReport, error) {
	if req.Declaration.Parser != "" {
		return parsePluginReport(req)
	}
	parser, err := BuiltinReportParserRegistry().Parser(req.Declaration.Format)
	if err != nil {
...
func parsePluginReport(req ParseRequest) (state.QualityReport, error) {
	plugin, ok := req.Plugins[req.Declaration.Parser]
	if !ok {
		return state.QualityReport{}, fmt.Errorf(
			"unknown quality parser plugin %q",
			req.Declaration.Parser,
		)
	}
	if plugin.Type != "quality" {
		return state.QualityReport{}, fmt.Errorf("plugin %q is not a quality plugin", plugin.Name)
	}
	...
	if plugin.Shell == "" && len(plugin.Command) == 0 {
		return state.QualityReport{}, fmt.Errorf("plugin %q must set command or shell", plugin.Name)
	}
	...
	if plugin.Shell != "" {
		...
		cmd = exec.CommandContext(ctx, plugin.Command[0], args...)
	}
	cmd.Dir = req.Workdir
	...
	if err := cmd.Run(); err != nil {
		...
		return state.QualityReport{}, fmt.Errorf("plugin %q failed: %w", plugin.Name, err)
	}
	parsed, err := decodePluginQuality(stdout.Bytes())
	if err != nil {
		return state.QualityReport{}, fmt.Errorf(
			"plugin %q emitted invalid quality JSON: %w",
			plugin.Name,
			err,
		)
	}
...
func decodePluginQuality(data []byte) (state.QualityReport, error) {
	...
	for key := range raw {
		if key != "metrics" && key != "findings" {
			return state.QualityReport{}, fmt.Errorf("unknown top-level field %q", key)
		}
	}
	if len(raw) == 0 {
		return state.QualityReport{}, fmt.Errorf("must include metrics or findings")
	}
	var report state.QualityReport
	if value, ok := raw["metrics"]; ok {
		if err := json.Unmarshal(value, &report.Metrics); err != nil {
			return state.QualityReport{}, fmt.Errorf("metrics: %w", err)
		}
	}
	if value, ok := raw["findings"]; ok {
		if err := json.Unmarshal(value, &report.Findings); err != nil {
			return state.QualityReport{}, fmt.Errorf("findings: %w", err)
		}
	}
	for _, metric := range report.Metrics {
		if metric.Name == "" {
			return state.QualityReport{}, fmt.Errorf("metric name is required")
		}
	}
	for _, finding := range report.Findings {
		if finding.Kind == "" {
			return state.QualityReport{}, fmt.Errorf("finding kind is required")
		}
	}
	return report, nil
}
