...
func (testTargetHandler) CompositeChildren(model.TargetBody) []CompositeChild { return nil }
...
func TestTargetRegistryZeroValue(t *testing.T) {
	var registry TargetRegistry
	if _, err := registry.Handler(model.TargetTypeShell); err == nil {
		t.Fatal("zero-value missing target handler returned no error")
	}
	if err := registry.Register(testTargetHandler{targetType: model.TargetTypeShell}); err != nil {
		t.Fatal(err)
	}
